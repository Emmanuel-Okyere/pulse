...
// Describe the redemption allowance in words.
function limitLabel(max: number): string {
  if (max === 0) return "unlimited redemptions";
  if (max === 1) return "single use";
  return `up to ${max} redemptions`;
}

export function RedeemPanel({ eventId }: { eventId: string }) {
  const [code, setCode] = useState("");
  const [lookup, setLookup] = useState<Registration | null>(null);
  ...
  async function find(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const res = await api<{ registration: Registration }>(
        `/api/registrations/${encodeURIComponent(code.trim())}/redeem`
      );
  ...
  async function redeem() {
    ...
    try {
      ...
      if (res.limitReached) {
        setMessage("This code has reached its redemption limit.");
      } else {
        setMessage("Redeemed. Benefit released to the attendee.");
      }
      setLookup((l) =>
        l
          ? {
              ...l,
              redeemed: true,
              redeemCount: res.redeemCount,
              redeemedAt: res.redeemedAt,
              canRedeem: res.canRedeem ?? false,
            }
          : l
      );
    } catch (err) {
        ...
        {lookup && (
          <div className="mt-5 rounded-xl border border-[var(--border)] p-4">
            <div className="flex items-center justify-between">
              ...
              {lookup.redeemed ? (
                <span className="chip bg-gray-100 text-gray-600">
                  redeemed{lookup.redeemCount > 1 ? ` ×${lookup.redeemCount}` : ""}
                </span>
              ) : (
            ...
            <div className="mt-1 font-mono text-xs text-ink-muted">
              {limitLabel(lookup.event.maxRedemptions)}
              {lookup.redeemedAt && (
                <> · last {new Date(lookup.redeemedAt).toLocaleString()}</>
              )}
            </div>
            ...
            {lookup.canRedeem ? (
              ...
              >
                {lookup.redeemed ? "Redeem again" : "Mark as redeemed"}
              </button>
            ) : (
              <div className="mt-4 rounded-xl bg-gray-50 px-3.5 py-2.5 text-center text-sm text-ink-muted">
